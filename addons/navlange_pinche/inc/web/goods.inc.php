...
if($op == 'index') {
    $travel = pdo_fetchall("SELECT * FROM " . tablename('navlange_pinche_travel') . " WHERE uniacid=" . $_W['uniacid'] . " AND type='5' ORDER BY release_time DESC");
    $travel_info = array();
    foreach ($travel as $key => $value) {
        $data = $value;
        if($data['type'] == '1' && $data['status']=='0' && $data['start_time'] < TIMESTAMP) {
