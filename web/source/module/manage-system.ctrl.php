...
if ($do =='install') {
	...
	$module['title_initial'] = get_first_pinyin($module['title']);//头字母的搜索索引

	if ($packet['schemes']){//cloud_m_prepare() 返回值 
		foreach ($packet['schemes'] as $remote) {
			$remote['tablename'] = trim(tablename($remote['tablename']), '`');
			$local = db_table_schema(pdo(), $remote['tablename']);
			$sqls = db_table_fix_sql($local, $remote);
			foreach ($sqls as $sql) {
				pdo_run($sql);//安装模块时的数据库脚本临时变量 ≈ install.php
			}
		}
	}
	...
	if (pdo_insert('modules', $module)) {//ims_modules 插入模块相关信息
		...
		cache_build_module_subscribe_type();//缓存更新操作
		cache_build_module_info($module_name);
		cache_build_uni_group();
		cache_delete(cache_system_key('user_modules', array('uid' => $_W['uid'])));
		if ($module_support_name == MODULE_SUPPORT_SYSTEMWELCOME_NAME) {//首页应用 支持检测
			itoast('模块安装成功！', url('module/manage-system/install_success', array('support' => $module_support_name)), 'success');
		}
